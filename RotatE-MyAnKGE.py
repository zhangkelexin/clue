...
def parse_args(args=None):
    ...
    parser.add_argument('--bs', default=1024, type=int)  # batch_size
    ...
    parser.add_argument('--refer_ent', default=0, type=float)  # 实体参考答案比例
    parser.add_argument('--refer_rel', default=0.5, type=float)  # 关系参考答案比例
    ...
    def __getitem__(self, idx):
        ...
        if self.flag == 'tail':
            if cfg.refer_ent != 0:
                ...
                y = torch.tensor([head, rel, tail]).view(-1, 3)
            if cfg.refer_rel != 0:
                if len(same_relation[rel.item()]) > 0:
                    r = torch.tensor(same_relation[rel.item()]).view(-1, 1)
                    h = torch.tensor([head]).view(-1, 1).expand(r.shape[0], 1)
                    t = torch.tensor([tail]).view(-1, 1).expand(r.shape[0], 1)
                    z = torch.cat([h, r, t], dim=-1)
                else:
                    z = torch.tensor([head, rel, tail]).view(-1, 3)
            else:
                z = torch.tensor([head, rel, tail]).view(-1, 3)
...
def evaluate(dataloader, already_dict, emb_model, dis, device, cfg, flag):
    ...
    with torch.no_grad():
        ...
        for pos, X, ankge, relation_ankge in dataloader:

            X = X.to(device).squeeze(0)
            ankge = ankge.to(device).squeeze(0)
            relation_ankge = relation_ankge.to(device).squeeze(0)
            h_emb, r_emb, t_emb = emb_model(X)  # print(h_emb.shape)  40943 * 2048
            h_emb_an, _, _ = emb_model(ankge)  # print(h_emb.shape)  n * 2048
            _, r_emb_an, _ = emb_model(relation_ankge)  # print(h_emb.shape)  n * 2048

            # 获取真实三元组中头实体的嵌入向量
            true_hr_emb = pos.clone().to(device)
            true_h_emb, true_r_emb, _ = emb_model(true_hr_emb)
            if cfg.refer_ent != 0:
                # 实体示例用法
                ...
                t_emb_tr = t_emb.repeat(h_emb_an.shape[0], 1)
                r_emb_tr = r_emb.repeat(h_emb_an.shape[0], 1)
                # 复制每一行n次
            ...
            if cfg.refer_ent != 0:
                # 实体
                pred_an, _ = dis(h_emb_an, r_emb_tr, t_emb_tr)
                pred_an = pred_an.view(-1, pred.shape[-1]).mean(dim=0)
                pred_an = (sim_score * pred_an).sum(dim=0)

            if cfg.refer_rel != 0:
                # 关系示例用法
                r_emb_an = find_top_k_similar_tensors(true_r_emb, r_emb_an, cfg.refer_num)
                sim_score_r = (true_r_emb * r_emb_an).sum(dim=-1)
                sim_score_r = F.softmax(sim_score_r, dim=0).view(len(sim_score_r), -1)
                sim_score_r = 1 - sim_score_r
                sim_score_r = torch.nn.functional.normalize(sim_score_r, p=1, dim=0)

                h_emb_tr = h_emb.repeat(r_emb_an.shape[0], 1)
                t_emb_tr = t_emb.repeat(r_emb_an.shape[0], 1)
                # 复制每一行n次
                r_emb_an = torch.cat([torch.unsqueeze(r_emb_an, 1)] * r_emb.shape[0], dim=1).view(-1, r_emb_an.shape[1])
                # 关系
                pred_an_r, _ = dis(h_emb_tr, r_emb_an, t_emb_tr)
                pred_an_r = pred_an_r.view(-1, pred.shape[-1]).mean(dim=0)
                pred_an_r = (sim_score_r * pred_an_r).sum(dim=0)

            if cfg.refer_ent != 0:
                pred += cfg.refer_ent * pred_an
            if cfg.refer_rel != 0:
                pred += cfg.refer_rel * pred_an_r
...
class DistMult111(torch.nn.Module):
    # ComplEx
    def forward(self, head, relation, tail):
        ...
        l2_reg = torch.mean(head ** 2) + torch.mean(relation ** 2) + torch.mean(tail ** 2)
...
def get_same_entities(test_data, already_ts_dict, already_hs_dict):
    ...
    with open('./dataset/same_entities.csv', 'w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        for key, value in same_entities.items():
...
def get_same_relations(test_data, already_rs_dict):
    ...
    with open('./dataset/same_relations.csv', 'w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        for key, value in same_relations.items():
            writer.writerow([key] + value)
...
if __name__ == '__main__':
    ...
    # 数据处理，返回：并集 和 交集
    # 数据保存过直接加载数据
    if file_is_exist('same_entities.csv'):
        same_entities = read_dict('same_entities.csv')
    else:
        # 重新加载数据
        same_entities = get_same_entities(test_data, already_ts_dict, already_hs_dict)
    # 数据存在直接加载数据
    if file_is_exist('same_relations.csv'):
        same_relation = read_dict('same_relations.csv')
    else:
        # 数据不存在重新加载数据
